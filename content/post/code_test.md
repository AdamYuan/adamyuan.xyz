...
{
	auto *data = new GLfloat[kWidth * kHeight * 4];
	std::fill(data, data + kWidth * kHeight * 4, 0.0f);
	glCreateTextures(GL_TEXTURE_2D, 1, &quad_tex_);
	glTextureStorage2D(quad_tex_, 1, GL_RGBA32F, kWidth, kHeight);
	glTextureSubImage2D(quad_tex_, 0, 0, 0, kWidth, kHeight, GL_RGBA, GL_FLOAT, data);

	glTextureParameteri(quad_tex_, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTextureParameteri(quad_tex_, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glTextureParameteri(quad_tex_, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTextureParameteri(quad_tex_, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glBindImageTexture(0, quad_tex_, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F);
	delete[] data;
	...
	glCreateTextures(GL_TEXTURE_2D, 1, &ray_.seed_tex);
	glTextureStorage2D(ray_.seed_tex, 1, GL_RGBA32UI, kWidth, kHeight);
	glTextureSubImage2D(ray_.seed_tex, 0, 0, 0, kWidth, kHeight, GL_RGBA_INTEGER, GL_UNSIGNED_INT, initial_seeds);

	glTextureParameteri(ray_.seed_tex, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTextureParameteri(ray_.seed_tex, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glTextureParameteri(ray_.seed_tex, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTextureParameteri(ray_.seed_tex, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
...
void Application::render_quad()
{
	...
	glBindTexture(GL_TEXTURE_2D, quad_tex_);
